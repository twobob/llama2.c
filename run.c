...
void generate(
	...
	if(saveFileBool){strcat_s(tokens_so_far, MAX_STRING_SIZE, piece);}
    ...
    if (pos > 1) {
        long end = time_in_ms();
        fprintf(stderr, "achieved tok/s: %f for %s\n", (pos-1) / (double)(end-start)*1000, COMPILER);
    }
...
int main(int argc, char *argv[]) {
    ...
    generate(&transformer, &tokenizer, &sampler, prompt, steps, saveFileBool, tokens_so_far, saveLogBool, timelog_file, dirname, timestamp, singleBOS, print_tokens,  emitRawTokenBool );
